...
public class TellrawConvIOFactory implements ConversationIOFactory {
     ...
     */
    public TellrawConvIOFactory(final BetonQuestLogger log, final ConfigAccessor config, final Plugin plugin,
    ...
    public ConversationIO parse(final Conversation conversation, final OnlineProfile onlineProfile) {
        return new TellrawConvIO(log, config, plugin, message, conversation, onlineProfile, colors);
    }
}
