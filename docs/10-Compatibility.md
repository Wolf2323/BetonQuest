...
A line can also represent a floating item. To do so enter the line as 'item:`ITEM`'. It will be replaced with the `Item` defined in the items.yml. Default amount is 1. In the above example, a floating map(when defined in items.yml) will be seen between two lines of text.
