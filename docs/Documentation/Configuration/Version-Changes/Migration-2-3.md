...
??? info "Automated Migration"
    *The migration is automated. You shouldn't have to do anything.*
    ...
    - conversations - options `condition`, `events`, `pointer` and `extend` are now `conditions`, `events`, `pointers` and `extends`
    
    <div class="grid" markdown>
    
    ```YAML title="Old Syntax"
    objectives:
      event: event LEFT ANY global persistent event:notify condition:sneak
    ```
    
    ```YAML title="New Syntax"
    objectives:
      event: event LEFT ANY global persistent events:notify conditions:sneak
    ```
...
Instead of using mmo specific pickup objectives or item conditions and evnets (now called actions) it now uses the standard implementations.
...
??? info "Automated Migration"
    *The migration is automated. You shouldn't have to do anything.*
    ...
    events:
      ...
      applyBonus: "score kill 1.2 event:multiply"
      bigProgress: "globalpoint progress *2 event:multiply"
    ```
