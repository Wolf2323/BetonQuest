...
public class ConditionVariable implements PlayerVariable {
    ...
    public String getValue(final Profile profile) throws QuestException {
        if (questTypeAPI.condition(profile, conditionId)) {
            return papiMode ? PlainTextComponentSerializer.plainText().serialize(pluginMessage.getMessage(profile, "condition_variable_met")) : "true";
        }
        return papiMode ? PlainTextComponentSerializer.plainText().serialize(pluginMessage.getMessage(profile, "condition_variable_not_met")) : "false";
    }
}
