...
public class ParsedSectionMessage extends ParsedMessage {
    ...
    private static Map<String, VariableString> parse(final VariableProcessor variableProcessor, final QuestPackage pack,
                                                     final ConfigurationSection section, final String path) throws QuestException {
        final ConfigurationSection messageSection = section.getConfigurationSection(path);
        if (messageSection != null) {
            return parseSection(variableProcessor, pack, messageSection);
        }
        final String messageString = section.getString(path);
        if (messageString != null) {
            final String raw = GlobalVariableResolver.resolve(pack, messageString);
    ...
    private static Map<String, VariableString> parseSection(final VariableProcessor variableProcessor,
                                                            final QuestPackage pack,
                                                            final ConfigurationSection messageSection) throws QuestException {
        final Map<String, VariableString> messages = new HashMap<>();
        for (final String key : messageSection.getKeys(false)) {
            final String raw = GlobalVariableResolver.resolve(pack, messageSection.getString(key));
            if (raw == null) {
