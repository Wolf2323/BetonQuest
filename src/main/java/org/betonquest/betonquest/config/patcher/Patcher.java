...
public class Patcher {
    ...
    public boolean patch(final ConfigurationSection config) {
        ...
        log.info("Current config version: " + version.getVersion());
        ...
        for (final Map.Entry<Version, List<Map<?, ?>>> patch : patches.entrySet()) {
            ...
            setConfigVersion(config, patch.getKey().getVersion());
        ...
        if (!patched) {
            if (patches.isEmpty()) {
                ...
                setConfigVersion(config, patches.lastEntry().getKey().getVersion());
            }
            return true;
        }
        return patched;
    ...
    private Version getConfigVersion(final ConfigurationSection config) {
        ...
        if (configVersion == null || configVersion.isEmpty()) {
            if (patches.isEmpty()) {
                return new Version(TECHNICAL_DEFAULT_VERSION);
            } else {
